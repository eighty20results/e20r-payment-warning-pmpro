	...
	class Example_Gateway_Addon extends E20R_PW_Gateway_Addon {
		...
		public function get_gateway_class_name( $gateway_name = null , $addon ) {
			...
			if ( !empty( $gateway_name ) && 1 !== preg_match( "/{$addon}/i", $this->get_class_name() ) ) {
				$utils->log("{$addon} not matching Example's expected gateway add-on");
				return $gateway_name;
			}
		...
		/**
		 * Check for a Example gateway license
		 *
		...
		public function check_licenses( $addon = null ) {
			...
			parent::check_licenses( $addon ); // TODO: Change the autogenerated stub
		}
