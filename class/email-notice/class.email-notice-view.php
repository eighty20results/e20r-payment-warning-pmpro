...
class Email_Notice_View {
	...
	public static function editor( $template_settings ) {
		...
		if ( ! current_user_can( apply_filters( 'e20rpw_min_settings_capabilities', 'manage_options' ) ) ) {
			wp_die( __( "You do not have sufficient permissions to access this page", Email_Notice::plugin_slug ) );
		}
		
		?>
		<h2><?php _e( 'Edit: Message Templates', Email_Notice::plugin_slug ); ?></h2>
		<table class="form-table">
			...
			<tr>
				<th scope="row" valign="top">
					<label for="e20r-message-templates"><?php _e( "Message Template", Email_Notice::plugin_slug ); ?></label>
				</th>
				<td>
					<select name="e20r-message-templates" id="e20r-message-templates">
						<option value="-1" selected="selected">
							<?php printf( '--- %s ---', __( 'Select a Message template to Edit', Email_Notice::plugin_slug ) ); ?>
						</option>
						...
						<?php } ?>
						<option value="new">
							<?php printf( '*** %s ***', __( 'Add new Message template', Email_Notice::plugin_slug ) ); ?>
						</option>
					</select>
				...
				<td class="e20r-hidden-template-settings">
					<table class="e20r-message-settings">
						...
						<form action="" method="post" enctype="multipart/form-data"
							...
							<?php wp_nonce_field( Email_Notice::plugin_prefix, 'message_template' ); ?>
							<?php
				...
				<td>
					<p class="submit">
						<input id="save-template-data" name="save-message-template" type="button" class="button-primary"
						       value="<?php _e( 'Save Template Settings', Email_Notice::plugin_slug ); ?>"/>
						<input id="reset-template-data" name="reset-message-template" type="button" class="button"
						       value="<?php _e( 'Reset', Email_Notice::plugin_slug ); ?>"/>
						<!--
                        <input id="add-new-template" name="add-message-template" type="button" class="button"
                               value="<?php _e( 'Add New Template', Email_Notice::plugin_slug ); ?>"/>
                        -->
					</p>
	...
	public static function add_template_entry( $template_name, $template, $return = false ) {
		...
		<tr class="e20r-start-hidden e20r-message-template-name_<?php esc_attr_e( $template_name ); ?>">
			<th scope="row" valign="top" class="e20r-message-template-col">
				<label for="e20r-message-type_<?php esc_attr_e( $template_name ); ?>">
					<?php _e( 'Message type', Email_Notice::plugin_slug ); ?>
				</label>
			</th>
			<td class="e20r-message-template-col">
				<select id="e20r-message-type_<?php esc_attr_e( $template_name ); ?>" name="e20r_message_template-type">
					<option value="-1" <?php selected( $template['type'], null ); ?>>
						<?php _e( 'Header/Footer', Email_Notice::plugin_slug ); ?>
					</option>
					<option value="expiration" <?php selected( $template['type'], 'expiration' ); ?>>
						<?php _e( 'Expiration Message', Email_Notice::plugin_slug ); ?>
					</option>
					<option value="recurring" <?php selected( $template['type'], 'recurring' ); ?>>
						<?php _e( 'Recurring Payment', Email_Notice::plugin_slug ); ?>
					</option>
					<option value="ccexpiration" <?php selected( $template['type'], 'ccexpiration' ); ?>>
						<?php _e( 'Credit Card Expiration', Email_Notice::plugin_slug ); ?>
					</option>
		...
		<tr class="e20r-start-hidden e20r-message-template-name_<?php esc_attr_e( $template_name ); ?> e20r-message-schedule-info">
			<th scope="row" valign="top" class="e20r-message-template-col">
				<label for="e20r-message-schedule_<?php esc_attr_e( $template_name ); ?>">
					<?php _e( 'Send schedule', Email_Notice::plugin_slug ); ?>
				</label>
			</th>
			<td class="e20r-message-template-col">
				<?php
					...
					foreach ( $template['schedule'] as $days ) { ?>
						<div class="e20r-schedule-entry">
							...
							<span class="e20r-message-schedule-label">
                                                        <?php _e( 'days before event', Email_Notice::plugin_slug ); ?>
                                                    </span>
							<span class="e20r-message-schedule-remove">
                                                        <input type="button"
                                                               value="<?php _e( "Remove", Email_Notice::plugin_slug ); ?>"
                                                               class="e20r-delete-schedule-entry button-secondary"/>
                                                    </span>
						</div>
					<?php }
					?>
					<button class="button-secondary e20r-add-new-schedule"><?php _e( "Add new schedule entry", Email_Notice::plugin_slug ); ?></button><?php
				} else {
					if ( in_array( $template_name, array( 'messagefooter', 'messageheader' ) ) ) {
						_e( "No schedule needed/defined", Email_Notice::plugin_slug );
					}
				} ?>
			</td>
		</tr>
		<tr class="e20r-start-hidden e20r-message-template-name_<?php esc_attr_e( $template_name ); ?>">
			<th scope="row" valign="top" class="e20r-message-template-col">
				<label for="e20r-message-template-subject_<?php esc_attr_e( $template_name ); ?>">
					<?php _e( 'Message subject', Email_Notice::plugin_slug ); ?>
				</label>
			</th>
			<td class="e20r-message-template-col">
		...
		<tr class="e20r-start-hidden e20r-message-template-name_<?php esc_attr_e( $template_name ); ?>">
			<th scope="row" valign="top" class="e20r-message-template-col">
				<label for="e20r-message-template-body_<?php esc_attr_e( $template_name ); ?>">
					<?php _e( 'Body', Email_Notice::plugin_slug ); ?>
				</label>
			</th>
			<td class="e20r-message-template-col">
				<div class="template_editor_container">
					...
					if ( false === $return && 'new' !== $template_name ) {
						...
						<button id="e20r-load-media-btn" type="button" class="button insert-media add_media" data-editor="e20r-message-body_new"><span class="wp-media-buttons-icon"></span><?php _e("Add Media", Email_Notice::plugin_slug ); ?></button>
						<!-- </div> -->
		...
		<tr class="e20r-start-hidden e20r-message-template-name_<?php esc_attr_e( $template_name ); ?>">
			...
			<td class="e20r-message-template-col">
				...
				<label>
					<span id="e20r-message-disabled-label"><?php _e( 'Disable this template?', Email_Notice::plugin_slug ); ?></span>
				</label>
				<p class="description small">
					<?php _e( 'This template will not be used or sent.', Email_Notice::plugin_slug ); ?>
				</p>
			</td>
		</tr>
		<tr class="e20r-message-template-name_<?php esc_attr_e( $template_name ); ?>" valign="top" scope="row">
			<th class="e20r-message-template-col">
				<label for="variable_references"><?php _e('Placeholder Reference', Email_Notice::plugin_slug ); ?>:</label>
			</th>
			<td>
				<div class="template_reference" style="background: #FAFAFA; border: 1px solid #CCC; color: #666; padding: 5px;">
					<p><em><?php _e('Insert these variables in email-notice window above.', Email_Notice::plugin_slug ); ?></em></p>
	...
	*/
	...
	/**
	 * Add the type of message metabox on the email-notice screen for Email Notices (e20r_email_message)
	 *
	 * @param array $types
	 */
	...
	/**
	 * Add the type of message metabox on the email-notice screen for Email Notices (e20r_email_notice)
	 */
	 ...
	 * @param string $type
	 *
	 * @since 1.0 - ENHANCEMENT: Uses static Email_Notice::default_variable_help() function
	 */
	public static function add_placeholder_variables( $type ) {
		...
		<dl>
			<?php
			$variables = apply_filters( 'e20r-email-notice-variable-help', array(), $type );
